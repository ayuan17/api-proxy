    ...
    $.ajax({
        ...
        $.ajax({
            ...
            for (var i = 0; i < response.businesses.length; i += 1) {
                ...
                var restaurantTitle = $("<h2 id='resName'>").text(response.businesses[i].name);
                var restaurantReview = $("<h3 id='review'>").text(response.businesses[i].rating + " stars out of 5");
                var restaurantPrice = $("<div id='rating'>").text(response.businesses[i].price);
                var restaurantImage = $("<img class='images'>");
